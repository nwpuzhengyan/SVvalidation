...
def write_read(chr,total_read,read_list,bp1,bp2,line,genes):
    for read_name in sorted(total_read):
        ...
        if bp>0:
            ...
            res.write('>'+str(line[0])+':'+str(line[1])+'-'+str(line[2])+':'+str(read_name)+':l'+'\n')
            res.write(read_seq[0]+'\n')
            res.write('>' + str(line[0]) + ':' + str(line[1]) + '-' + str(line[2]) + ':' + str(read_name) + ':r'+'\n')
...
def read_bam(samfile,chr,pos1,pos2):
    ...
    for read in samfile.fetch(chr, pos1-100, pos2+100):
        if read.query_name in read_list:
            if read.mapq > read_list[read.query_name][0] and read.is_supplementary==False and\
                read.reference_start < pos1 - 500 and read.reference_end > pos2 + 500 and read.mapq>30:
            ...
            if read.is_supplementary==False and read.reference_start<pos1-500 and read.reference_end>pos2+500\
                    and read.mapq>30:
                read_list[read.query_name] = [read.mapq, read.tags[2][1], read.reference_start,read.reference_end,\
                                          read.is_secondary,read.cigarstring,read.query_sequence,read.query_length]
...
samfile2=pysam.AlignmentFile(args.hg19_del_bam, 'rb')
...
while line:
    ...
    print(line)
    chr = str(line[0])
    read_list = read_bam(samfile1,chr, int(line[1]), int(line[2]))
    total_read = list(set(sorted(read_list.keys())))
    DEL_read_list = read_bam(samfile2, chr, int(line[3]), int(line[3]))
    for k in DEL_read_list.keys():
        if k in total_read:
            if  DEL_read_list[k][1]<read_list[k][1]:
                del DEL_read_list[k]
            else:
                del read_list[k]
    total_read = list(set(sorted(read_list.keys())))
    ...
    write_read(chr, total_read, read_list, m_pos, m_pos, line, genes1)
    total_read = list(set(sorted(DEL_read_list.keys())))
    print(len(total_read))
    write_read(chr, total_read, DEL_read_list, int(line[3]), int(line[3]), line, genes2)
    line = f.readline()
f.close()
res.close()