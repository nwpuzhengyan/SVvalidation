...
parser.add_argument("INS", help='DEL file')
...
while line:
    line = line.split()
    print(line)
    res.write(str(line[0]) + ':' + str(line[1]) + '-' + str(line[1]) + ' ')
    chr = str(line[0])
    read_list = read_bam(samfile,chr,int(line[1]),int(line[2]), int(line[3]))
    total_read = list(set(sorted(read_list.keys())))
    read_num = calculate_read_num(chr, total_read, read_list,int(line[2]), int(line[3]))
    res.write(read_num + '\n')
    line = f.readline()
f.close()
res.close()
